...
- **各レイヤーの厳密な責務:**
  ...
  - **Serviceレイヤー:**
    - **責務:** Service層は、以下の3つの主要な責務を担う「ビジネスの頭脳」である。
      ...
      2.  **トランザクション管理:** 複数のDB操作を`Unit of Work`パターンなどを通じて一つの安全な処理単位として保証し、データの一貫性を守る。
      3.  **プレゼンテーション層向けのデータ変換:** Repositoryから受け取った永続化モデル（DBのデータ構造）を、Controller/Viewが扱いやすい専用の形式（DTOやDataFrame）に変換する。
    - **原則:**
...
最終合意したスコープに基づき、以下のスプリント計画でリファクタリングを推進します。

### スプリント 1 & 2: Repository層の構築
- **目標:** DBへのデータアクセス層を構築する。`Create`/`Delete`を実装し、`Read`のインターフェースを定義する。
- **主なタスク:**
    1. `BaseRepository`クラスを作成する。
    2. 各モデルに対応するRepositoryクラスを作成し、`Create`と`Delete`メソッドを実装する。
    3. `Read`系メソッドのインターフェース（器のみ）をダミー実装で定義する。
    4. `Create`/`Delete`機能の単体テストを作成する。

### スプリント 3: Serviceレイヤーの構築
- **目標:** ビジネスロジック層を構築し、UIとデータアクセス層を仲介する準備を整える。
- **主なタスク:**
    1. `FinancialService`クラスを作成し、`Repository`を呼び出す構造を実装する。
    2. `Read`時は、`Repository`のダミーメソッドを呼び出しつつ、CSVから読み取ったデータをUIに返すロジックを実装する。
    3. `Create`/`Delete`時は、実装済みの`Repository`メソッドを呼び出すロジックを実装する。

### スプリント 4: UIとServiceの連携
- **目標:** UI(`app.py`)が`Service`層のみを呼び出す形にリファクタリングし、責務分離を完了させる。
- **主なタスク:**
    1. `app.py`から`FinancialService`を呼び出し、返却されたデータを表示するように修正する。
    2. `Service`層の`Read`系ロジック（CSV読み取り部分）に対する単体テストを作成する。

### スプリント 5: データ永続化フローと品質向上
- **目標:** データをDBに保存・削除する流れを完成させ、テストで品質を担保する。
- **主なタスク:**
    1. データ永続化（`Create`）と削除（`Delete`）の一連のフローを`Service`経由で実装・テストする。
    2. `Create`/`Delete`に関する`Service`層の単体テスト（Repositoryはモック）を拡充する。

### スプリント 6: 最終確認とクリーンアップ
- **目標:** プロジェクト全体が意図通りに動作することを確認し、リファクタリングを完了させる。
- **主なタスク:**
    1. アプリケーション全体を通しての動作確認（データ永続化、UI表示など）。
    2. 不要になった古いコード（`db_controller.py`など）を削除する。
    3. `README.md`の実行手順などを最新の状態に更新する。
